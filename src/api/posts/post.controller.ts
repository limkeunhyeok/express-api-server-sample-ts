...
export default class PostController implements Controller {
  ...
  initializeRoutes() {
    ...
    router
      .post("/", validationMiddleware(CreatePostDto), wrap(this.createPost))
      .get("/", wrap(this.findAll))
      .get(
        "/:postId",
        validationMiddleware(ReadPostByPostIdDto),
        wrap(this.findOneByPostId)
      )
      .put(
        "/:postId",
        validationMiddleware(UpdatePostDto),
        wrap(this.updatePost)
      )
      .delete(
        "/:postId",
        validationMiddleware(DeletePostByPostIdDto),
        wrap(this.deleteOneByPostId)
      );
    // TODO
  ...
  deleteOneByPostId: Handler = async (req): Promise<IPostDocument> => {
    ...
    const deletedPost: IPostDocument = await this.postService.deleteOneByPostId(
      params
    );
