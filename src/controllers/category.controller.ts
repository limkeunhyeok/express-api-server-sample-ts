...
export default class CategoryController {
  ...
  public findOneById: Handler = async (req) => {
    ...
    const category: Category = await this.categoryService.findOneByCategoryId(categoryId);
    return category;
  }
  ...
  public update: Handler = async (req) => {
    ...
    const category: Category = await this.categoryService.updateCategory(categoryId, params);
    return category;
  }

  public delete: Handler = async (req) => {
    const categoryId: string = req.params.id;
    const category: Category = await this.categoryService.deleteCategory(categoryId);
    return category;
  }
}